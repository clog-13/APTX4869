...
public class Main {
    ...
    void run() {
        ...
        while (sc.hasNext()) {
            ...
            long res = 0;
            for (Node n: nodes) {
                update(1, get(n.y1), get(n.y2), n.z * n.k);
    ...
    void update(int u, long start, long end, long val) {
        if (tr[u].ri < start || tr[u].le > end) return;  // start和end始终相差 H
        if (start <= tr[u].le && tr[u].ri <= end) {  // 当前节点 区间长度小于 H，且在s-e范围内
            tr[u].flag += val;
            tr[u].sum += val;
        } else {
            pushDown(u);
            update(u<<1, start, end, val);
            update(u<<1|1, start, end, val);  // 当前节点 区间长度大于 H 或 不在s-e范围内
