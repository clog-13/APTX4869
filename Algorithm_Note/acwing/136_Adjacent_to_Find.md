...
对于 A 中的每一个数 Ai，求： min1≤j<i|Ai−Aj|
...
class Main {
    ...
    void run() throws IOException {
        ...
        for (int i = 1; i <= N; i++) {
            arr[i] = new Node(Integer.parseInt(str[i-1]), i);
        }
        Arrays.sort(arr, 1, N+1, (a, b) -> (int) (a.val - b.val));
        ...
        for (int i = N; i > 1; i--) {  // 倒序遍历每个数
            int j = map[i], le = les[j], ri = ris[j];  // j:arr排序后在arr的下标位置
            long lv = Math.abs(arr[j].val - arr[le].val);
            long rv = Math.abs(arr[ri].val - arr[j].val);
            if (lv <= rv) res[i] = new Node(lv, arr[le].id);  // 比较出最小值
