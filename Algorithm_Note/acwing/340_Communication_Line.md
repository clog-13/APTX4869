...
class Main {
    static int maxN = 1010, maxM = 30010, INF = 0x3f3f3f3f;
    ...
    private static void spfa() {
        ...
        while (!queue.isEmpty()) {
            ...
            for (int i = info[cur]; i != -1; i = from[i]) {
                // max(dis[cur][c], val[i]) 新加入的边需要掏钱的,
                // 所以就要和前面走过的边所用的费用进行对比,选取中最大的费用;
                // 因为当前点花钱, 所以前面点最多可以选c条免费(c越大,dis只会越小)
                // dis[cur][c-1] 新加入的边免费升级,
                // 则新加入一条边需要支付的费用，还是前面的费用没有改变;
                // 就是 dis[t][c] = dis[cur][c-1];
                
                // min(dis[cur][c-1], max(dis[cur][c], val[i]))
                // 新加入的这边，免费修和不免费修，两种方案的费用中取最小值;
                int t = to[i];
                for (int c = 0; c <= K; c++) {
                    int min = 0;
                    ...
                    if (dist[t][c] > min) {
                        dist[t][c] = min;  // dis[i][j]: i点, j条免费 的最小费用
                        if (!vis[t]) {
                            vis[t] = true;
                            queue.add(t);
                        }
