...
public class Main {
    ...
    private static boolean dfs(int cnt, int curLen, int start) {
        if(cnt * res == sum) return true;
        if(curLen == res) return dfs(cnt+1,0,N-1);

        for (int i = start; i >= 0; i--) {  // 剪枝3-1,i从start开始枚举
            if (vis[i]) continue;
            if (curLen+arr[i] > res) continue; // 可行性剪枝

            vis[i] = true;
            if (dfs(cnt,curLen+arr[i],i-1)) return true;
            vis[i] = false;

            if(curLen == 0) return false;    // 剪枝3-3:如果是木棒的第一根木棍失败了，则一定失败，退出
            if(curLen+arr[i] == res) return false; // 剪枝3-4:如果是木棒的最后一根木棍失败了，则一定失败，退出

            int j = i;
            while(j >= 0 && arr[i] == arr[j]) j--;  // 剪枝3-2:如果当前木棍加到当前棒中，失败了，则直接略过后面所有长度相等的木棍
            i = j+1;
        }
        return false;
    }
