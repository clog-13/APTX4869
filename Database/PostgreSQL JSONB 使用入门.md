# [PostgreSQL JSONB 使用入门](https://segmentfault.com/a/1190000019344353)

## json 类型说明

根据[RFC 7159](https://link.segmentfault.com/?enc=AMPbkuyribmjb%2Fuw1mg4BA%3D%3D.nA4T4SRaJV%2BKw%2FhjdTxeUT5%2BOZq6W4ckas2%2BblInO7noM3yLvSr0xPq1b%2BlTnDh%2B)中的说明，JSON 数据类型是用来存储 JSON（JavaScript Object Notation）数据的。这种数据也可以被存储为`text`，但是 JSON 数据类型的优势在于能强制要求每个被存储的值符合 JSON 规则。也有很多 JSON 相关的函数和操作符可以用于存储在这些数据类型中的数据
...
在把文本 JSON 输入转换成jsonb时，JSON的基本类型（[RFC 7159 ](https://link.segmentfault.com/?enc=3Iugso7S7hH76JtFSDq6UQ%3D%3D.BcOgbLQK%2BnNBqPNuzzDmwkzFLe4LaG1Un2avmfC2jHSfkEywEe0c%2FHVldcE9Ps7U)）会被映射到原生的 PostgreSQL类型。因此，jsonb数据有一些次要额外约束。
...
![img](https://segmentfault.com/img/remote/1460000019344356)
...
----------
 t
-- 会抛出错误，这里无论匹配项是text类型的 'python'  还是 json 类型的 '"python"'::json都会抛出异常
-- json 类型不支持 等号（=）操作符
select '{"nickname": "gs", "avatar": "avatar_url", "tags": ["python", "golang", "db"]}'::json#>'{tags,0}' = '"python"';
ERROR:  operator does not exist: json = unknown
```
...
![img](https://segmentfault.com/img/remote/1460000019344357)
...
详细的json 函数和操作符可以参考文档：[JSON 函数和操作符](https://link.segmentfault.com/?enc=KjipEawdJIYY7CLhk0Q8iQ%3D%3D.1%2FsTESgJ%2BXpIg0e5NP1oYqKBdsXGP4php7wrQL9IdePxJuURRha9yR9lhi6hv2gDBs633JpUMkCe%2BZrJkaEYSA%3D%3D)
...
> GIN(Generalized Inverted Index, 通用倒排索引) 是一个存储对(key, posting list)集合的索引结构，其中key是一个键值，而posting list 是一组出现过key的位置。如(‘hello', '14:2 23:4')中，表示hello在14:2和23:4这两个位置出现过，**s**
>
...
![gin 索引数据结构](https://segmentfault.com/img/remote/1460000019344358)
